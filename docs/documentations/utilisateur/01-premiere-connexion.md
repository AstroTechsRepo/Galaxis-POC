...
> Le tour du propriétaire en 10 minutes.
...
- **L'adresse de votre Galaxis** — quelque chose comme `http://localhost:9080` (en démo) ou `https://galaxis.votre-entreprise.fr` (en vrai)
- **Votre identifiant** — par exemple `marc`
- **Votre mot de passe initial** — par exemple `Demo2026!` (à changer dès la première connexion !)
...
1. Dans le champ **"Nom d'utilisateur ou e-mail"** : tapez votre identifiant (ex : `marc`)
2. Dans le champ **"Mot de passe"** : tapez votre mot de passe
3. Cliquez sur le bouton **"Se connecter"** (ou appuyez sur Entrée)

> **Attention** : si votre mot de passe ne marche pas, vérifiez :
...
│   ┌────────────────────────────────────┐  │
│   │ sub : abc-123-...                  │  │
│   │ email : marc@atelier-marchand.demo │  │
│   │ …                                  │  │
│   └────────────────────────────────────┘  │
└────────────────────────────────────────────┘
```
...
Si votre mot de passe est un mot de passe générique donné par votre prestataire, **changez-le tout de suite** :
...
> **Attention** : un mot de passe sûr doit faire **au moins 12 caractères**, mélanger majuscules/minuscules, chiffres et symboles. Vaultwarden peut générer un mot de passe fort pour vous (voir chapitre 03).
