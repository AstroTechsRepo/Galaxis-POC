...
> ⚠️ **Note POC** : dans la version actuelle, cette étape se fait dans la **console d'administration Keycloak**. C'est un écran plus technique que le reste de Galaxis. Si vous n'êtes pas à l'aise, demandez à votre prestataire de vous accompagner la première fois.

### Accéder à la console admin

1. Ouvrez Galaxis dans le navigateur
2. Ajoutez `/iam/admin` à l'URL → `http://localhost:8080/iam/admin`
...
🔗 Adresse : https://galaxis.votre-tpe.fr (ou http://localhost:8080 si POC)
👤 Identifiant : sophie
...
> ⚠️ **Important** : Sophie devra créer un **mot de passe maître Vaultwarden** différent de son mot de passe Galaxis. Préviens-la et insiste : *"Tu ne dois jamais oublier ce mot de passe maître."*
