...
Le backend autorise **uniquement `APP_URL`** (par défaut `http://localhost:8080`).
...
⚠️ **Volontairement absents en POC** :
...
| `no-new-privileges` | ❌ | ✅ |
| User non-root dans le conteneur | ⚠️ partiel (Vaultwarden, Nextcloud officielles) | ✅ obligatoire partout |
| `cap_drop: [ALL]` | ❌ | ✅ |
| Scan vulnérabilités images (Trivy) | ❌ | ✅ en CI/CD |

> 💡 Pour durcir le POC : ajouter `security_opt: [no-new-privileges:true]` à chaque service applicatif.
...
⚠️ Volontairement basique — pour la prod, brancher CloudWatch / Loki avec rétention 90+ jours et alarmes (ex : > 10 `auth.rejected` du même IP en 1 min).
