...
flowchart LR
    ...
    smoke["Smoke test<br/>curl 127.0.0.1:9080"]:::s
...
- `docker compose up -d` (services IAM)
- Attend `curl /realms/master` (health check) jusqu'à 60×3s = 3 min
...
**Attention** : **si vous changez le mot de passe admin Keycloak après le premier déploiement**, le script échouera car il s'authentifie avec l'ancien. Procédure : ouvrir la console Keycloak en navigateur (par tunnel SSH), changer le password, puis adapter le `.env`.
...
Idempotent. Couvre :
- `docker compose up -d` (services Vaultwarden + Nextcloud + DB + Caddy)
- Smoke test : `curl http://127.0.0.1:9080/` doit renvoyer 200 (ou 302)
