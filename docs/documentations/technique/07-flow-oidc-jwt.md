...
sequenceDiagram
    ...
    R->>K: 302 /iam/.../auth?<br/>response_type=code&<br/>client_id=galaxis-portal&<br/>redirect_uri=/auth/callback&<br/>code_challenge=...&<br/>code_challenge_method=S256&<br/>scope=openid profile email&<br/>state=xyz
    K-->>U: page login Keycloak
    U->>K: identifiants
    K->>K: vérifie credentials + brute force protection
    K-->>R: 302 /auth/callback?code=ABC&state=xyz

    R->>K: POST /iam/.../token<br/>grant_type=authorization_code&<br/>code=ABC&<br/>code_verifier=...&<br/>client_id=galaxis-portal&<br/>redirect_uri=/auth/callback
    ...
    L->>L: iss == http://localhost:8080/iam/realms/galaxis ?
...
- **Cache miss / `kid` inconnu** : on fetch `/iam/.../certs` et on remplit le cache
...
// iss
if ($payload['iss'] !== "http://localhost:8080/iam/realms/galaxis") {
...
curl -i http://localhost:8080/api/me -H "Authorization: Bearer this.is.not.a.jwt"
...
| Tous les calls 401 après rotation Keycloak | Cache JWKS vieux mais kid inchangé | Manuel : `docker exec galaxis-app-redis redis-cli -a $REDIS_PASSWORD DEL galaxis:jwks:keys` |
