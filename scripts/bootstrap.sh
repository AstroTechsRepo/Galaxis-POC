...
# Endpoint de health Keycloak — exposé par caddy-iam en HTTP sur 8080
KC_HEALTH_URL="${KC_HEALTH_URL:-http://localhost:8080/realms/master}"
...
start=$(date +%s)
while true; do
  if curl -fsS -o /dev/null --connect-timeout 3 "${KC_HEALTH_URL}" 2>/dev/null; then
    ok "Keycloak READY"
    break
  fi
  now=$(date +%s)
...
PUBLIC_ORIGIN="${PUBLIC_ORIGIN:-http://localhost:9080}" \
...
echo ""
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  Galaxis POC v1.2 est prêt — scénario Atelier Marchand       ║"
echo "║                                                              ║"
echo "║  Depuis le laptop :                                          ║"
echo "║    ssh -L 8080:127.0.0.1:8080  -L 9080:127.0.0.1:9080  \\    ║"
echo "║        -L 10180:127.0.0.1:10180 -L 11180:127.0.0.1:11180 \\ ║"
echo "║        user@<vm-ip>                                          ║"
echo "║                                                              ║"
echo "║  Puis ouvrir dans le navigateur :                            ║"
echo "║    http://localhost:9080   → Portail Galaxis                 ║"
echo "║    http://localhost:8080   → Keycloak admin                  ║"
echo "║    http://localhost:10180  → Vaultwarden                     ║"
echo "║    http://localhost:11180  → Nextcloud                       ║"
echo "║                                                              ║"
