# LIVRAISON — Galaxis POC v1.2-soutenance
...
| **Repo Git** | `https://github.com/AstroTechsRepo/Galaxis-POC` |
...
| Caddyfiles | [`deployments/caddy/`](./deployments/caddy/) | Caddy 2.8-alpine (3 instances : iam, app, services) |
...
- Fichier : [`backend/tests/Feature/JwtMiddlewareTest.php`](./backend/tests/Feature/JwtMiddlewareTest.php) — **9 tests** middleware JWT
...
- Fichier : [`backend/tests/Feature/DemoSeederTest.php`](./backend/tests/Feature/DemoSeederTest.php) — **9 tests** seeder demo
- **Total : 23 tests** (coverage cible ≥ 60%)
...
### Documentation PROJET — 10 chapitres
...
| 10 | [IA et LLM comme levier](./docs/documentations/projet/10-ia-methodologie.md) |
...
# ou ouvrir manuellement http://localhost:9080
```
...
> **Attention : POC démo uniquement.** Ce mot de passe partagé n'est pas une pratique de production. Il est documenté ici et dans `docs/documentations/demo-guide.md`, **jamais commité dans `.env`**. Il est injecté à l'exécution via la variable `DEMO_PASSWORD` (cf. `infrastructure/scripts/configure-keycloak.sh`).
...
| 2 | Tunnel SSH + `http://localhost:9080` → portail visible avec DA | ✅ |
| 3 | Login `marc / Demo2026!` → dashboard avec claims | ✅ |
...
| 14 | PDF livrable unifié (si pandoc dispo) | optionnel |
...
8. 3 reverse proxies Caddy HTTP (ports 8080, 9080, 10180, 11180) + tunnel SSH
...
- **33 tests automatisés** couvrant la sécurité et le flow critique
- **3 documentations** complètes (28 chapitres + 3 index + 1 guide démo)
...
Le tout dans un repo Git versionné et taggé `v1.2-soutenance`.
